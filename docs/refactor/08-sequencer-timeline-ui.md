...
| 轨道导航栏 | Timeline 左侧 | 260px 宽 | 搜索、添加轨道、轨道树、分组折叠、轨道状态 |
| 时间轴画布 | Timeline 右侧 | 剩余空间 | 标尺、片段、关键帧、Marker、播放头、横向滚动 |
| 传输控制栏 | Timeline 底部 | 34px 高 | 跳转、逐帧、播放/暂停、速度、循环 |

### 轨道导航栏

- 顶部提供 `+ Track` 按钮和轨道搜索框；没有对应业务命令的创建操作显示禁用态，并通过 tooltip 说明“等待后端接口”。
- 轨道树按视频轨道、相机轨道、Marker 轨道进行分组；组可折叠，折叠后右侧画布同步隐藏子行。
- 每条轨道左侧显示类型图标和名称，右侧显示可见、锁定、静音等状态；当前没有后端命令的状态控件只展示实际状态，不产生伪操作。
- 导航栏与画布使用同一份可见轨道行序列、行高和垂直滚动偏移，确保左右严格对齐。

### 时间轴画布

- 标尺固定于画布顶部，按缩放级别显示刻度和不小于 14px 的时间标签。
- 视频片段、相机关键帧、Marker 和转场共用以 tick 为单位的水平坐标系。
...
- 不实现视频导出、编码、离线渲染或导出任务调度。
- 不实现尚无业务接口的轨道创建、删除、锁定、静音、循环区间提交。
- 不实现新的回放数据格式；继续使用 `EditorStateExt`、`EditorBridge` 和已有命令栈。
