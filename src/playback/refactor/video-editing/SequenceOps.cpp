...
void bindCamera(editor::SequenceSegment& segment, const std::string& cameraId) { segment.cameraId = cameraId; }
void clearDanglingRefs(std::vector<editor::SequenceSegment>& segments, const std::string& removedCameraId) { for (auto& segment : segments) if (segment.cameraId == removedCameraId) segment.cameraId.clear(); }
}
